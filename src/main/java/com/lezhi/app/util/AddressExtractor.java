...
public class AddressExtractor {
    ...
    private static final Pattern __buildingRegex = Pattern.compile(
            "(?<b>[\\d0-9a-zA-Z]*?[东西南北上中下甲乙丙丁戊己庚辛壬癸一二三四五六七八九十]*?)" +
                    "(?:号楼?|单元|幢|楼|座)" +
    ...
    private static String extractRoomNo(String input) {
        ...
        //甲乙丙丁置前会与楼栋号的标记混淆
        Pattern p = Pattern.compile("(?<r0>[\\d\\-_a-zA-Z一二三四五六七八九十/\\\\]+[甲乙丙丁戊己庚辛壬癸]?)室(?<r4>[a-zA-Z甲乙丙丁戊己庚辛壬癸]?)");
        ...
        if (n == null) {
            p = Pattern.compile("(?<r10>(?:\\d+层)?全幢)室?");
            m = p.matcher(input);
            if (m.find()) {
                n = m.group("r10");//"全幢";
            }
        ...
        if (n == null) {
            p = Pattern.compile("(?<r1>[\\d\\-_a-zA-Z一二三四五六七八九十]+[层楼])$");
            m = p.matcher(input);
            while (m.find()) {  //出现多次要最后一个
                n = m.group("r1");
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(parseAll("金山区蒙山路1090弄24号602室：602室"));
    }
    ...
    private static String preProcess(String line) {
        ...
        pattern = Pattern.compile(".+?[a-zA-Z\\d]+号([a-zA-Z\\d_一二三四五六七八九十]+层).+室");
        matcher = pattern.matcher(line);
        if (matcher.find()) {
            line = line.replaceFirst(matcher.group(1), "");
        }
